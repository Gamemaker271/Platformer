...
// Detect the platform, to enable platform-specific code
    ...
    #define TGUI_SYSTEM_EMSCRIPTEN // Emscripten
#elif defined(_WIN32)
...
// GCC and clang supported std filesystem before version 9, this is the first version where no
...
// Xcode 11 supports it on macOS 10.15 only, so we require Xcode 12 which doesn't run on macOS 10.14.
    ...
    || (defined(__GNUC__) && (__GNUC__ >= 9)) || (defined(_MSC_VER) && (_MSC_VER >= 1914))
...
// TGUI_NEXT: Remove TGUI_EMPLACE_BACK, TGUI_IF_CONSTEXPR and TGUI_NODISCARD defines, they should no longer be used
...
#define TGUI_NODISCARD [[nodiscard]]
    ...
    #define TGUI_IGNORE_DEPRECATED_WARNINGS_START _Pragma("warning(push)") _Pragma("warning(disable: 4996)")

    #define TGUI_IGNORE_DEPRECATED_WARNINGS_END _Pragma("warning(pop)")
#elif defined(_MSC_VER)
    #define TGUI_IGNORE_DEPRECATED_WARNINGS_START __pragma(warning(push)) __pragma(warning(disable : 4996))
